#!/usr/bin/env python3
"""Update Excel Creator function to handle both string and array inputs."""
import json
import time
import os
...
# Updated content that handles both string format AND JSON array format
...
class Tools:
    ...
    def create_excel(self, title: str, headers, data) -> str:
        ...
        :param headers: Column headers - either comma-separated string "Product,Jan,Feb,Mar" or list ["Product","Jan","Feb","Mar"]
        :param data: Row data - either pipe-separated string "Widget,100,150,200|Gadget,250,300,350" or nested list [["Widget",100,150,200],["Gadget",250,300,350]]
        ...
        try:
            ...
            return "Error: openpyxl not installed. Please install it with: pip install openpyxl"
        ...
        if isinstance(headers, str):
            ...
            return f"Error: headers must be string or list, got {type(headers)}"

        # Handle data - can be string or nested list
        if isinstance(data, str):
            ...
            return f"Error: data must be string or list, got {type(data)}"
        ...
        html = f"""
<div style="padding:20px;background:linear-gradient(135deg,#1e3a5f,#2d5a87);border-radius:12px;color:white;margin:10px 0;">
...
# Verify
cursor.execute('SELECT id, name, type, updated_at FROM function WHERE id = %s', (function_id,))
row = cursor.fetchone()
print(f"Function in database: {row}")
