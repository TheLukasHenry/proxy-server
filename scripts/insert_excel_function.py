#!/usr/bin/env python3
"""Insert Excel Creator function into Open WebUI database."""
import sqlite3
import json
import time
...
user_id = "f1bcfe21-28b6-4bab-855b-0bfa30a31d6e"
name = "Excel Creator"
func_type = "tool"
...
if existing:
    print(f"Function {function_id} already exists, updating...")
    cursor.execute('''
        UPDATE function SET name=?, type=?, content=?, meta=?, updated_at=?, is_active=?, is_global=?
        WHERE id=?
    ''', (name, func_type, content, meta, now, 1, 1, function_id))
else:
    print(f"Inserting new function {function_id}...")
    cursor.execute('''
        INSERT INTO function (id, user_id, name, type, content, meta, created_at, updated_at, valves, is_active, is_global)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (function_id, user_id, name, func_type, content, meta, now, now, '{}', 1, 1))

conn.commit()
print(f"Committed! Rows affected: {cursor.rowcount}")

# Verify
cursor.execute('SELECT id, name, type, is_active, is_global FROM function')
